...
import defeat
from tiles import Sand_tile, Sand_tile_1
...
class Level_b_2:
    def __init__(self, level_data, surface):
        ...
        #Background
        self.background = pygame.image.load('Resourses/Backgrounds/Beach/Beach_background.png').convert()
    ...
    def setup_level(self, layout):
        ...
        for row_index, row in enumerate(layout):
            for col_index, cell in enumerate(row):
                ...
                if cell == 'X':
                    beach_tile = Sand_tile((x,y),tile_size)
                    self.tiles.add(beach_tile)
                if cell == 'P':
                    player_sprite = Players((x, y))
                    self.player.add(player_sprite)
                if cell == 'Y':
                    beach_tile_1 = Sand_tile_1((x,y),tile_size)
                    self.tiles.add(beach_tile_1)
    ...
    def run(self):
        ...
        #Verify if the counter reaches 110
        if self.trash_collected >= 110:
        ...
        #If there are at least 95 trash on the ground, close the game
        if trash_on_floor >= 95:
        ...
        text_trash_collected = self.get_font(18).render(f"{self.trash_collected}/110",True, (255,255,255))
        #Trash on the ground counter
        text_trash_on_ground = self.get_font(18).render(f"{trash_on_floor}/95", True, (255,255,255))
